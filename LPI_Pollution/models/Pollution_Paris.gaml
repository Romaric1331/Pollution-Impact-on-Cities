...
/** Insert the global definitions, variables and actions here */
	...
	init {
		...
		the_graph <- main_connected_component(the_graph);
		...
		list<building> residential_building <- building where (each.type = "apartments");

		// list of industrial building
		list<building> industrial_building <- building where (each.type != "apartments");
...
species people skills: [moving] {
	...
	reflex time_to_work when: the_target = nil {
		objective <- "working";
...
//	}
	...
	reflex move when: the_target != nil {
		...
		if the_target = location {
			...
			//adding pollution
			// list<geometry> segments <- current_path.segments copy_between (0, current_path.segments index_of current_edge);
